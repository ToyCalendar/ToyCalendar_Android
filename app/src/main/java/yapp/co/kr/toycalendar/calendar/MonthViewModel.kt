...
class MonthViewModel(val monthData: MonthData, calendarRepository: CalendarRepository) {
    ...
    private fun createDays(schedules: List<Schedule?>?): List<Day> {
        ...
        for (i in 0 until numOfDays) {
            ...
            val date = SimpleDateFormat("yyyyMMdd", Locale.getDefault()).format(
                GregorianCalendar(
                    year,
                    month,
                    day
            ...
            list.add(
                createDay(
                    ...
                    day,
                    dayOfWeek,
                    schedules.findLast { it?.date.equals(date) })
            )
            day++
            dayOfWeek++
        ...
        for (i in 0 until firstDayOfMonth) list.add(0, Day().apply {
            //TODO 달력의 마지막 날에 생리나 배란이 겹쳤을 때 선을 그어주기 위한 코드인데,
            //지금 일단 선은 개발하지 않으니 스킵
...
//                if(list[list.lastIndex].ovulationStartYn || list[list.lastIndex].ovulationCycleYn) ovulationCycleYn = true
    ...
    private fun createDay(
        ...
        val result = Day(false, year, month, day, dayOfWeek, schedule?.dayType ?: DayType.NONE)
